"use client";
{/* DEBUG: 탭 버전 확인 */}
<div className="px-5 pb-2 text-xs font-black text-pink-600">
  TAB MODAL ✅
</div>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md rounded-3xl bg-white/80 backdrop-blur-xl shadow-2xl border border-black/5 overflow-hidden">
        <div className="px-5 py-4 flex items-center justify-between">
          <div>
            <div className="text-xs font-extrabold tracking-widest text-pink-500/80">
              TOP 20
            </div>
        ...
        <div className="px-5 pb-5">
            ...
            <div className="mb-3 rounded-2xl bg-pink-50 border border-pink-100 px-4 py-3">
              ...
              {myRank !== undefined && (
                <div className="mt-2 text-xs font-bold text-slate-700">
                  Your Rank:{" "}
                  <span className="text-pink-600 font-black">#{myRank}</span>{" "}
                  {!inTop20 && (
                    <span className="text-slate-500 font-semibold">(Top20 밖)</span>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="rounded-2xl bg-white border border-black/5 overflow-hidden">
            <div className="grid grid-cols-[52px_1fr_84px] px-4 py-3 text-xs font-extrabold text-slate-500 bg-slate-50">
              <div>RANK</div>
              <div>NICK</div>
                ...
                {rows.map((r) => {
                  ...
                  return (
                    <div
                      ...
                      className={`grid grid-cols-[52px_1fr_84px] px-4 py-3 border-t border-black/5 text-sm ${
                        isMe ? "bg-pink-50" : ""
                      }`}
                    >
                      ...
                      <div className="font-bold text-slate-800 truncate">
                        {r.nickname}
                        {isMe ? (
                          <span className="ml-2 text-xs font-black text-pink-600">YOU</span>
                        ) : null}
                        {r.date ? (
